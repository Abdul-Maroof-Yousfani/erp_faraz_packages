...
<div class="row well_N align-items-center">
    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
        <ul class="cus-ul">
            ...
            <li>
                <h3><span class="glyphicon glyphicon-chevron-right"></span> &nbsp;Edit Machine</h3>
            </li>
        </ul>
    </div>
                                                                ...
                                                                <div class="col-sm-8">
                                                                    <select name="asset" onchange="getAsset()" class="form-control select2 requiredField" id="asset">
                                                                        ...
                                                                        @foreach (CommonHelper::get_all_asset() as $row)
                                                                                <option value="{{$row->id.','.$row->asset_name}}" {{$Machine->asset_id == $row->id ?  'selected' : ''}}>{{$row->asset_name}}({{$row->asset_code}})</option>
                                                                        @endforeach
                                                                    </select>
                                                                    <input name="asset_id" id="asset_id"  class="form-control" type="hidden">
                                                                    <input name="name" id="name"  class="form-control" type="hidden">
                                                                    {{-- <input name="name" id="name" value="{{ $Machine->name }}" class="form-control" type="text"> --}}
                                                                </div>
                                                            </div>
                                                        </div>
