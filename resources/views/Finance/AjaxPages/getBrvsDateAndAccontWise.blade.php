    ...
    <td class="text-center hidden-print">
        <div class="dropdown">
            ...
            <ul class="dropdown-menu">
                <li>
                        <?php if($view == true):?>
                        <a onclick="showDetailModelOneParamerter('fdc/viewBankRvDetailNew','<?php echo $row1->id;?>','View Bank Reciept Voucher Detail','<?php echo $m?>','')" class="">View</a>
                        <?php endif;?>

                        <?php if($row1->rv_status ==1):?>
                        <?php if($edit == true):?>
                        <a href="<?php echo  URL::to('/finance/editBankRv/'.$row1->id.'?m='.$m); ?>" type="button" class=" BtnHide<?php echo $row1->rv_no?>">Edit</a>

                        <?php endif;?>
                        <?php endif;?>
                                @if($delete==true)
                            <a class=" BtnHide<?php echo $row1->rv_no?>" type="button"onclick="DeleteRvActivity('<?php echo $row1->id;?>','<?php echo $row1->rv_no?>','<?php echo $row1->rv_date?>','<?php echo CommonHelper::GetAmount('new_rv_data',$row1->id)?>')">Delete</a>
                            @endif
                        ...
                        {{--<input class="btn btn-xs btn-danger" type="button" onclick="DeletePvActivity('< ?php echo $row1->id;?>')" value="Delete" />--}}
                            <a onclick="change_colour('{{$row1->id}}')" target="_blank" href="<?php echo url('fdc/viewBankRvDetailNewPrint?id='.$row1->id.'&&m='.$m)?>" class="">Print</a>
