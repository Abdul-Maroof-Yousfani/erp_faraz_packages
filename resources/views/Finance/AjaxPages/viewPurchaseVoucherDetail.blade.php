...
#printPurchaseVoucherDetail table.tableMargin > tbody > tr > td:first-child{color:#6c5ce7;font-weight: 500;background:#f7f7fc;}
#printPurchaseVoucherDetail table.tableMargin > tbody > tr:nth-child(even){background:#eef0fa;}
#printPurchaseVoucherDetail table.tableMargin > tbody > tr:nth-child(even) td{border-bottom:1px solid #1e2757;color:#1e2757;font-weight: 500;}
...
#printPurchaseVoucherDetail .table-responsive table thead th,#printPurchaseVoucherDetail table.tra thead th,#printPurchaseVoucherDetail table.tra tr:first-child th{background:#eef0fa;color:#444a6b;font-weight: 500;font-size:13px;padding:10px 12px;border:none;border-bottom:1px solid #dfe2f2;}
...
#printPurchaseVoucherDetail .table-responsive table tbody tr td label{color:#6b7094;font-weight: 500;}
...
/* Show Voucher checkbox label */
 label.check{display:inline-block;margin:14px 0;font-weight: 500;color:#444a6b;}
/* Signature / approval boxes */
 #printPurchaseVoucherDetail .signature_bor{color:#000;font-weight: 500;font-size:13px;border-top:2px solid #1e2757;padding-top:10px;margin-top:40px;}
...
@media print{
    ...
    #printPurchaseVoucherDetail .signature_bor{border-top:2px solid #1e2757 !important;color:#000;font-size:12px!important;padding-top:8px!important;margin-top:30px!important;}
}
...
<div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-right">
        ...
        <button type="button" class="btn btn-xs btn-success"
            onclick="ApproveVoucher('<?php            echo $id?>')">Approve</button>
        <?php        endif;?>
        <?php
    endif;
    ...
    echo $Supplier->name;
                                    ...
                                    ?>
                                        <tr class="text-center">
                                            ...
                                            <td title="{{$row2->sub_item}}">
                                                {{ CommonHelper::get_item_name($row2->sub_item) }} </td>
                                            <td>
                                                <?php        echo CommonHelper::get_uom($row2->sub_item);?>
                                            </td>
                                            <td><?php        echo $row2->qty?></td>
                                            <td><?php        echo $row2->rate?></td>
                                            <td>PKR <?php        echo $row2->amount; ?></td>
                                            <!-- <td><?php        echo number_format($item_amount_percent, 3); ?></td> -->
                                            <td>PKR {{  number_format($exp_amount_apply, 2) }}</td>
                                            <!-- <td><?php        echo $row2->discount_amount; ?></td> -->
                                            <td>PKR <?php        echo number_format($row2->net_amount + $exp_amount_apply, 2);
        $TotalAmount += $row2->net_amount + $exp_amount_apply;?>
                                            </td>
                                        </tr>
                                        <?php
    }
                                    ?>
                                        <tr>
                                            ...
                                            <td class="text-center"><b>PKR <?php    echo number_format($TotalAmount, 2)?></b>
                                            </td>
                                            <input type="hidden" id="Total" value="<?php    echo $TotalAmount?>">
                                            <input type="hidden" name="d_t_amount_1" id="d_t_amount_1" value="{{ $row->sales_tax_amount + $TotalAmount }}">
                                        </tr>
        ...
        $Accounts = CommonHelper::get_single_row('accounts', 'id', $row->sales_tax_acc_id);
                                    ?>
                                        <tr>
                                        <td colspan="7" class="text-center">
                                                <label><b>{{ optional($Accounts)->name }}</b></label>
                                            </td>
                                            <td class="text-center">
                                                <b>PKR <?php  echo number_format($row->sales_tax_amount, 2)?></b></td>
                                        </tr>
                                        <tr>
                                            <td colspan="7" class="text-center"><b>Total After Sales Tax<b></td>
                                            <td class="text-center">
                                                <b>PKR <?php  echo number_format($row->sales_tax_amount + $TotalAmount, 2)?></b>
                                        ...
                                        <?php    endif;?>
                                        <tr>
                                            <td colspan="8" class="text-center" id="rupees"></td>
    ...
    $aditional_exp = DB::Connection('mysql2')->table('new_purchase_voucher_data')->where('master_id', '=', $id)->where('additional_exp', 1)->get(); ?>
                                        ...
                                        @foreach($aditional_exp as $row3)
                                            <tr>
                                                ...
                                                <td class="text-center">{{CommonHelper::get_account_name($row3->category_id)}}</td>
                                                <td class="text-center">PKR {{number_format($row3->net_amount, 2) }}<?php        $TotalAddional += $row3->net_amount;?>
                                                </td>
                                            </tr>
                                        @endforeach
                                ...
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        ...
                                        <div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 text-center">
                                            ...
                                            <b>
                                                <p><?php    echo strtoupper($approve_1) ?></p>
                                            </b>
                                        </div>
                                        <div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 text-center">
                                            <h6 class="signature_bor">Approved By:</h6>
                                            <b>
                                                <p><?php    echo strtoupper($approve_2)?></p>
                                            </b>
                                        </div>
                                        <div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 text-center">
