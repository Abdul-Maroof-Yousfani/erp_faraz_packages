...
<div>
    ...
    <input type="hidden" name="store_challan_no" id="store_challan_no" value="<?php echo $store_challan_id->store_challan_no ?>"/>
...
<div class="row">
    ...
    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
        ...
        <select onchange="warehouseFromChange(this.value)" name="warehouse_from_id" id="warehouse_from_id" class="form-control requiredField">
            ...
            @foreach(CommonHelper::get_all_warehouse() as $row)
                <option @if( $store_challan_id->warehouse_from_id == $row->id ) SELECTED @endif value="{{ $row->id }}">{{ ucwords($row->name) }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
        ...
        <select name="warehouse_to_id" id="warehouse_to_id" class="form-control requiredField">
            {{-- <option value="">Select Warehouse</option> --}}
            @foreach(CommonHelper::get_all_warehouse() as $row)
                <option @if( $store_challan_id->warehouse_to_id == $row->id ) SELECTED @endif value="{{ $row->id }}">{{ ucwords($row->name) }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
        ...
        <span class="rflabelsteric"><strong>*</strong></span>
        <input type="date" class="form-control requiredField" name="store_challan_date" id="store_challan_date" value="{{ $store_challan_id->store_challan_date }}" />
    </div>
...
<div class="row">    
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <span class="rflabelsteric"><strong>*</strong></span>
        <textarea name="main_description" id="main_description" rows="2" cols="50" style="resize:none;" class="form-control">{{ $store_challan_id->description }}</textarea>
    </div>
</div>
			...
			<table class="table table-bordered sf-table-list">
                ...
                ?>
                        ...
                        <tr id="storeChallanDetailRow_<?php echo $gmrddRow->id?>">
                            ...
                            @php
                                $batch_codes = ReuseableCode::get_bacth_code($store_challan_id->warehouse_from_id,$gmrddRow->sub_item_id)
                            @endphp
                            <td class="text-center">
                                <select onchange="getStockBatchWise({{$gmrddRow->id}}, {{$gmrddRow->sub_item_id}}, this.value)" class="form-control batchcode" name="batch_code_{{$gmrddRow->id}}" id="batch_code_{{$gmrddRow->id}}">
                                    <option value="">Select Batch</option>
                                    @foreach ($batch_codes as $batch_code)
                                        <option @if( $stock[$key]->batch_code == $batch_code->batch_code) selected @endif  value="{{ $batch_code->batch_code }}">{{ $batch_code->batch_code }}</option>                                        
                                    @endforeach
                                </select>
                            </td>
                            ...
                            <td>
                                <input type="text" name="store_challan_qty_<?php echo $gmrddRow->id?>" id="store_challan_qty_<?php echo $gmrddRow->id?>" class="form-control store_challan_qty requiredField" value="{{ $store_challan_data_id[$key]->issue_qty }}" onkeyup="checkAvailableBalance(this.id,this.value,<?php echo $key?>);checkqty(this.id,this.value, {{ $gmrddRow->qty }},<?php echo $key?>)" />
                            </td>
                            <td>
...
<script type="text/javascript">
    ...
    {   
        ...
        let uri = "{{ url('/') }}"+"/repository/getStockBatchWise"
        $.ajax({
            type: "get",
            url: uri,
            ...
            success: function (response) {
                ...
                html = `
                    <span>${response.current_stock} ${response.batch_detail.subitem_uom}</span><br>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Trigger the onchange event for the element with ID "mySelect"
        let batchcode = document.querySelectorAll('.batchcode')
        let store_challan_qty = document.querySelectorAll('.store_challan_qty')
    
        for (let index = 0; index < batchcode.length; index++) {
            batchcode[index].dispatchEvent(new Event('change'));
            store_challan_qty[index].dispatchEvent(new Event('keyup'));
        }
