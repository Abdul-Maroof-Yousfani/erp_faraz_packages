                ...
                <div class="row">
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="well">
                            ...
                            <div class="row">
                                <?php echo Form::open(array('url' => 'pad/updateStockTransfer?m='.$m.'','id'=>'addPurchaseReturnDetail','class'=>'stop'));?>
                                                        ...
                                                        <table class="table table-bordered sf-table-list">
                                                            ...
                                                            <tr class="text-center">
                                                                ...
                                                                <th class="text-center">
                                                                    <span class="badge badge-success" id="span"><?php echo $Detail->count();?>/span>
                                                                </th>
                                                            </tr>
                                                            <tr>
                                                            ...
                                                            <tr class="text-center AutoNo" id="RemoveRow<?php echo $Couter?>">
                                                                <td>
                                                                    ...
                                                                    <input type="hidden" class="requiredField requiredField" name="item_id[]" id="sub_<?php echo $Couter?>" value="<?php echo $Fil->item_id?>">
                                                                        <input type="text" name="item_idd[]" id="item_<?php echo $Couter?>" class="form-control sam_jass requiredField" value="<?php echo $ItemArray->sub_ic?>">
                                                                </td>
                                                                <td>
                                                                    <select onchange="get_stock(this.id,'<?php echo $Couter?>','No')" name="warehouse_from[]" id="warehouse_from<?php echo $Couter?>" class="form-control requiredField" style="width: 180px;">
                                                                        <option value="">Select Warehouse</option>
                                                                        <?php foreach(CommonHelper::get_all_warehouse() as $FFil):?>
                                                                        <option value="<?php echo $FFil->id?>" <?php if($Fil->warehouse_from == $FFil->id):echo "selected"; endif;?>><?php echo $FFil->name?></option>
                                                                        <?php endforeach;?>
                                                                    </select>
                                                                </td>

                                                                <td><select onchange="get_stock_qty(this.id,'{{$Couter}}')" class="form-control requiredField" name="batch_code[]" id="batch_code{{$Couter}}">
                                                                        <option value="">Select&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</option>
                                                                        <option selected value="{{$Fil->batch_code}}">{{$Fil->batch_code}}</option>

                                                                    </select></td>
                                                                <td>
                                                                    <input value="{{ReuseableCode::get_stock($Fil->item_id,$Fil->warehouse_from,0,$Fil->batch_code)}}" type="text" name="in_stock_qty[]" id="in_stock_qty<?php echo $Couter?>" class="form-control requiredField" readonly>
                                                                </td>
                                                                <td>
                                                                    <input onkeyup="check_qty(this.id,'<?php echo $Couter?>')" type="number" name="qty[]" id="qty<?php echo $Couter?>" class="form-control requiredField SendQty" step="any" value="<?php echo $Fil->qty?>">
                                                                </td>
                                                                <td style="display: none">
                                                                    <input readonly type="number" name="rate[]" id="rate<?php echo $Couter?>" class="form-control" value="<?php echo $Fil->rate?>">
                                                                </td>
                                                                <td style="display: none">
                                                                    <input readonly type="number" name="amount[]" id="amount<?php echo $Couter?>" class="form-control" value="<?php echo $Fil->amount?>">
                                                                </td>
                                                                <td>
                                                                    <select name="warehouse_to[]" id="warehouse_to<?php echo $Couter?>" class="form-control requiredField" style="width: 180px;">
                                                                        <option value="">Select Warehouse</option>
                                                                        <?php foreach(CommonHelper::get_all_warehouse() as $TFil):?>
                                                                        <option value="<?php echo $TFil->id?>" <?php if($Fil->warehouse_to == $TFil->id): echo "selected"; endif;?>><?php echo $TFil->name?></option>
                                                                        <?php endforeach;?>
                                                                    </select>
                                                                </td>
                                                                <?php if($Couter > 1):?>

                                                                    <td><button type="button" class="btn btn-xs btn-danger" id="BtnRemove<?php echo $Couter?>" onclick="RemoveRows('<?php echo $Couter?>')">-</button></td>
                                                                <?php else:?>
                                                                    <td>-</td>
                                                                <?php endif?>
    ...
    <script>
        ...
        function AddMoreRows()
        {
            ...
            $('#AppendHtml').append('<tr class="text-center AutoNo" id="RemoveRow'+Counter+'">' +
                    ...
                    '</td>' +
                    '<td>' +
                    '<select onchange="get_stock(this.id,'+Counter+',"No")" name="warehouse_from[]" id="warehouse_from'+Counter+'" class="form-control" style="width: 180px;">' +
                    '<option value="">Select Warehouse</option>'+
                    ...
                    '</td>' +
                    '<td>' +
                    '<input onkeyup="check_qty(this.id,'+Counter+')" type="number" name="qty[]" id="qty'+Counter+'" class="form-control requiredField SendQty">' +
                    '</td>' +
                    '<td style="display: none">' +
            ...
            $('.SendQty').on('keypress', function (event) {
        ...
        function get_stock(warehouse,number,Condition)
        {
