                                            ...
                                            <div class="row qout-h" style="padding: 10px">
                                                ...
                                                <div class="col-md-12 padt pos-r">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-2">
                                                            <label for="">Selected Printed Roll</label>
                                                            <input 
                                                                type="text"  
                                                                readonly
                                                                id="roll_item_name"
...
$global_avg_amt=0;
    ...
    @forelse($out_source_productions_details as $key => $detail)
        @php 
            $allowed_qty = $detail->no_of_roll - ($detail->used_qty ?? 0);
            
            $rate = App\Helpers\CommonHelper::getAccurateFIFOAvgRate($detail->item_id, $out_source_productions_item->date);

            $global_avg_amt += $rate * $detail->no_of_roll;   // sum of (rate × qty)
        $global_avg_qty += $detail->no_of_roll;           // sum of qty
        @endphp
        <tr>
            <td>{{ $key+1 }}</td>
            <td>{{ $detail->item_id }}</td>
            <td>{{ $detail->no_of_roll }}</td>
            ...
            <td>{{ number_format($detail->no_of_roll * $rate, 2) }}</td>
        </tr>
    @empty
        <tr>
...
@endphp
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-2">
                                                            <label for="">Used No. of Printed Rolls</label>
                                                            <input 
                                                                type="text"
                                                                id="used_qty_total"
                                                                name="used_qty_total"
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-1">
                                                            ...
                                                            <input 
                                                                ...
                                                                name="no_of_roll[]"
                                                                id="no_of_roll_1"
                                                                class="form-control move-next no_of_roll_1 requiredField"
                                                                onkeyup="cal_amt()"
                                                                required 
                    ...
                    <div class="row" id="row_${count}">
                        ...
                        <div class="col-md-1">
                            ...
                            <input 
                                type="text" 
                                name="no_of_roll[]"
                                id="no_of_roll_${count}"
                                class="form-control move-next no_of_roll_${count} requiredField"
                                onkeyup="cal_amt()"
                                required 
...
function cal_amt() {
    ...
    // Get all no_of_roll fields
    let no_of_roll_inputs = $("input[name='no_of_roll[]']");
    let printed_no_of_roll_inputs = $("input[name='printed_no_of_roll[]']");
    let count = no_of_roll_inputs.length;
    ...
    if (count > 0 && used_qty_total > 0 && event?.type === "keyup" && $(event.target).hasClass("used_qty_total")) {
        let no_of_roll_avg = used_qty_total / count;
        no_of_roll_inputs.each(function () {
            $(this).val(no_of_roll_avg.toFixed(2));
        });
        
    }

    // ---- CASE 2: Validate manual no_of_roll edits ----
    let sum_no_of_roll = 0;
    no_of_roll_inputs.each(function () {
        sum_no_of_roll += parseFloat($(this).val()) || 0;
    });

    if (sum_no_of_roll > used_qty_total) {
...
$("[id^='no_of_roll_']").each(function (index) {
    let row = index + 1;

    let qty = parseFloat($(`#no_of_roll_${row}`).val()) || 0;
