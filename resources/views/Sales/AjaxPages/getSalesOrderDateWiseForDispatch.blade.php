...
@foreach($sale_order as $row)
    ...
    <tr @if ($row->so_type==1) style="background-color: lightyellow" @endif title="{{$row->id}}" id="{{$row->id}}">
        ...
        <td class="text-center">{{strtoupper($row->order_no)}} </td>
        <td class="text-center">{{strtoupper($row->dc_no)}} </td>
        <td class="text-center">{{strtoupper($row->packing_list_no)}} </td>
        <td class="text-center">{{strtoupper($row->no_bundle??0)}} </td>
        <td class="text-center"><?php  echo CommonHelper::get_item_name($row->item_id);?></td>
        ...
        <td class="text-center"><button
                    onclick="dispatch('<?php echo $row->sod_id?>','<?php echo $m ?>' , '{{$row->p_id}}')"
                    type="button" class="btn btn-primery btn-xs">Create Dispatch</button></td>
    </tr>
