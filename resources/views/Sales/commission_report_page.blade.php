...
@section('content')
                ...
                <div class="dp_sdw">    
                    <div class="row">
                        <div class="">
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                <span class="subHeadingLabelClass">Commission Report</span>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                            ...
                            <select onchange="getCommissionColumn()" name="AgentId" id="AgentId" class="form-control select2" >
                                <option value="">Select Agent</option>
                                <?php foreach($Agent as $Fil):?>
                                <option value="<?php echo $Fil->id?>"><?php echo $Fil->agent_name?></option>
                                <?php endforeach;?>
                            </select>
                        </div>
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                            ...
                            <select onchange="get_commission_data_ajax()" name="MultiValues" id="MultiValues" class="form-control select2" ></select>
                            <span id="Loading"></span>
                        </div>

                        <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 text-left">
                            <input type="button" value="Submit" class="btn btn-sm btn-primary" onclick="get_commission_data_ajax()" style="margin-top: 32px;" />
                        </div>
                    </div>
    ...
    <script>
        ...
        function get_commission_data_ajax()
        {
            var MultiValues = $('#MultiValues').val();

            var m = '<?php echo $m?>';
            $('#ShowHide').html('<div class="row"><div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"><div class="loader"></div></div></div>');

            $.ajax({
                url: '/sdc/get_commission_data_ajax',
                type: 'Get',
                data: {MultiValues: MultiValues,m:m},
        ...
        function getCommissionColumn()
        {
            ...
            {
                var m = '<?php echo $m?>';
                $('#Loading').html('<div class="row"><div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"><div class="loader"></div></div></div>');

                $.ajax({
                    url: '/sdc/getCommissionColumn',
                    type: 'Get',
                    data: {AgentId: AgentId,m:m},

                    success: function (response) {
                        $('#MultiValues').html(response);
                        $('#Loading').html('');
                    }
                });
            }
