                ...
                <div class="dp_sdw">
                    <div class="row align-items-center">
                        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                            <div class="headquid">
                                <h2 class="subHeadingLabelClass">Formulation List</h2>
                            </div>
                        </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="userlittab table table-bordered sf-table-list" id="EmpExitInterviewList">
                                            <thead>
                                                ...
                                                <th class="text-center">Formulation No</th>
                                                <th class="text-center">Status</th>
                                                <th class="text-center">Action</th>
                                            </thead>
                                            <?php $count = 0; ?>
                                            <tbody id="data">
                                                @foreach ($recipeList as $Fil)
                                                <tr id="remove<?php echo $Fil['id'] ?>">
                                                    <td>{{++$count}}</td>
                                                    <td>{{$Fil->subItem->sub_ic ?? ''}}</td>
                                                    <td> {{$Fil->receipe_name}} </td>
                                                    <td> {{$Fil->formulation_no}} </td>
                                                    <td>
                                                        @if($Fil->status == 1)
                                                            Active 
                                                        @elseif($Fil->status == 2)
                                                    ...
                                                    <td class="text-center">
                                                        <div class="dropdown">
                                                            ...
                                                            <ul class="dropdown-menu">
                                                                <li>
                                                                    <a onclick="showDetailModelOneParamerter('recipe/viewRecipeInfo','{{$Fil->id}}','View Recipe Detail')" type="button" class="" >
                                                                     View</a>
                                                                    <a href="recipeEdit?id=<?php echo $Fil['id'] ?>&&m=<?php echo $this->m?>">  Edit</a>
                                                                    <a target="_blank" href="{{ url('/recipe/addRecipe') }}?copy_id={{$Fil->id}}&m=<?php echo $this->m?>">  Copy</a>
                                                                    @if($Fil->status == 1)
                                                                        <a onclick="showDetailModelOneParamerter('recipe/changeFormulationStatus','{{$Fil->id.'--'.'2'}}','','{{$this->m}}')" >Disable</a>
                                                                    @elseif($Fil->status == 2)
                                                                        <a onclick="changeFormulationStatus('{{$Fil->id}}','1','{{$this->m}}')" >Enable</a>
                                                                    @endif
                                                                </li>
                                                            </ul>
                                                        </div>
