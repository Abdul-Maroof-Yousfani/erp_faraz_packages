...
?>
    ...
    <td>{{ $cur }} {{ number_format($row->amount + $row->gst_amount,2) }}</td>
    <td class="text-center">{{ QuotationHelper::quotationStatus($row->quotation_status) }}</td>
    <td>
        <div class="dropdown">
            ...
            <ul class="dropdown-menu">
                <li>
                    ...
                    @if ($edit == true && $row->quotation_status == 1)
                        <a  href="{{ url('quotation/edit_quotation/'.$row->pr_id.'/'.$row->id) }}" class="">Edit</a>
                    @endif
                    @if ($delete == true)
                        <a onclick="delete_quotation('{{$row->id}}')" type="button" class="">Delete</a>
                    @endif
                    @if ($summary == true)
