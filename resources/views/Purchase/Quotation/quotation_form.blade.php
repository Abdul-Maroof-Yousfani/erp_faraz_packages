            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="well_N">
                    <div class="dp_sdw">    
                        ...
                        <div class="row">
                            {{ Form::open(array('url' => url('quotation/insert_quotation').'?m='.$m, 'id' => 'quotationForm', 'class' => 'stop')) }}
                                    ...
                                    <div class="panel-body">
                                        <div class="row">
                                            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                                                ...
                                                <input readonly type="text" class="form-control requiredField" placeholder="" name="pr_no" id="pr_no" value="{{strtoupper($voucher_no)}}" />
                                            </div>
                                                    ...
                                                    <table class="table table-bordered">
                                                        ...
                                                        @foreach($request_data as $key => $row)
                                                            <tr class="text-center">
                                                                <td>
                                                                    <input type="checkbox" name="checked_rows[]" value="{{ $key }}" id="check{{ $key }}">
                                                                </td>
                                                                <td>{{ $count++ }}</td>
                                                                <td>{{ CommonHelper::get_item_name($row->sub_item_id) }}</td>
                                                                <td>{{ CommonHelper::get_uom($row->sub_item_id) }}</td>
                                                                <td>{{ $row->qty }}</td>
                                                                <td><input onkeyup="calcu('{{ $key }}','{{ $row->qty }}')" onblur="calcu('{{ $key }}','{{ $row->qty }}')" class="form-control zerovalidate requiredField" step="0.001" type="number" name="rate[]" id="rate{{ $key }}"/> </td>
                                                                <td><input readonly  class="form-control zerovalidate requiredField amount" step="0.01" type="number" name="amount[]" id="amount{{ $key }}"/> </td>
                                                                <input type="hidden" name="pr_data_id[]" value="{{ $row->id }}"/>
                                                            </tr>
                                                        @endforeach
                                                                    ...
                                                                    <td colspan="3">
                                                                        ...
                                                                        <select style="width:100%" onchange="sales_tax(this.id)" class="form-control select2" id="sales_taxx" name="sales_taxx">
                                                                            ...
                                                                            @foreach(ReuseableCode::get_all_sales_tax() as $row)
                                                                                <option data-sale-tax="{{ $row->rate }}" value="{{ $row->id }}">{{ $row->percent }}</option>
                                                                            @endforeach
                                                                        </select>
                                                                    </td colspan="3">
                                                                    <td class="text-right" >
