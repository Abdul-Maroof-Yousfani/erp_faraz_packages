...
(function (window, document, $) {
  ...
  $(window).on('load', function () {
            ...
            // storage code
      let localValue = localStorage.getItem('tabValue');
      let inner_tabValue = localStorage.getItem('inner_tabValue');
      if(localValue !== null){
        let sideNav = document.getElementById('mySidenav').childNodes;
        // console.log(sideNav);
        for(let i = 0; i < sideNav.length; i++){
          if(i % 2){
            $(sideNav[i]).children().find(`button[data-target=${localValue}]`).click()
            // .addClass("active");
            // if($(sideNav[i]).find("button").attr("data-target") === localValue){
            //   $()
            // }
          }
          // let attribute = $(`${sideNav[i]} ul li div button`).getAttribute('data-target');
          // console.log(attribute);
        }
      }

      setTimeout(() => {
        $(localValue).children().find(`a[data-target=${inner_tabValue}]`).click()
      }, 1000)

			$('.sm-bx button').on('click', function(){
        let button = this;
        let tabValue = button.getAttribute('data-target');
        console.log(tabValue);
        localStorage.setItem('tabValue', tabValue);
		  })

      $('.list-unstyled a[href=#]').on('click', function(){
        let button = this;
        let tabValue = button.getAttribute('data-target');
        console.log(tabValue);
        localStorage.setItem('inner_tabValue', tabValue);
		  }) 
