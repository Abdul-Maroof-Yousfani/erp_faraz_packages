...
//        $sales_order_data=$sales_order_data->where('master_id',$id)->get();
    ...
    public function EditDeliveryNote()
    {
        ...
        $delivery_note_data = DB::connection('mysql2')
            ...
            ->select(
                ...
                'b.qty as bqty',
                'b.bundle_unit',
                's.sub_ic', 's.uom', 's.item_code', 'u.uom_name','pt.type','s.pack_size','s.primary_pack_type','s.color'
            )
            ->where('a.master_id', $id)
            ->groupBy('a.groupby')
            ->get();
    ...
    public function CreateDispatch(Request $request)
    {
        ...
        ->select('dc.dc_no' , 'pp.order_no' , 'p.packing_list_no' , 's.sub_ic' , 'so.so_no' , 'sc.sub_category_name'  , 'so.description'  ,'u.uom_name' ,
        'dc.id as dc_id' , 'pp.id as pp_id' , 'p.id as p_id'  , 'mr.id as mr_id', 's.id as s_id' , 'so.id as so_id' , 'p.customer_id' , 'p.customer_name' , 'dcd.qty')
...
//        print_r($delivery_note_data); die();
        ...
        $packing = DB::Connection('mysql2')->table('packings as p')
        ->select('dc.dc_no' , 'pp.order_no' , 'p.packing_list_no' , 's.sub_ic' , 'so.so_no' , 'sc.sub_category_name'  , 'so.description'  ,'u.uom_name' ,
        'dc.id as dc_id' , 'pp.id as pp_id' , 'p.id as p_id'  , 'mr.id as mr_id', 's.id as s_id' , 'so.id as so_id' , 'p.customer_id' , 'p.customer_name')
        ...
        $behavior = 'Approve 1';
            ...
            foreach ($sales_tax_invoice_data as $key => $row):
              ...
              if ($qty <0):
                ...
                $average_cost=ReuseableCode::average_cost_sales($row->item_id,
                $row->warehouse_id,
                0);
