...
//        $suppliers = $suppliers->where('status', 1)->select('name','acc_id' ,'id', 'email', 'resgister_income_tax', 'company_name', 'business_type', 'cnic', 'ntn', 'filer')->get();
        ...
        if(!empty($search)) {
            ...
            $suppliers = DB::Connection('mysql2')->select('select s.*,s.id s_id, c.id as customer_id from supplier s
                left join customers c on (c.acc_id = s.acc_id or (LOWER(TRIM(c.name)) = LOWER(TRIM(s.name)) and LOWER(TRIM(s.name)) != "")) and c.status = 1
                where s.status=1 and (
                    LOWER(COALESCE(s.name, "")) like LOWER(?) or
                    LOWER(COALESCE(s.contact_person, "")) like LOWER(?) or
                    COALESCE(s.mobile_no, "") like ?
                )
                group by s.id
        ...
        foreach ($suppliers as $row) {
            ...
            <tr id="<?php echo $row->s_id ?>">
                ...
                <td class="text-center"><?php echo (!empty($row->customer_id) || (!empty($row->mark_as_customer) && (int)$row->mark_as_customer === 1)) ? 'Yes' : 'No'; ?></td>
                <td class="text-center">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <li>
                                <a onclick="showDetailModelOneParamerter('purchase/viewSupplierFormlist/<?php echo $row->id ?>','<?php echo $row->id ?>','View Supplier Detail')" target="_blank" class="dropdown-item_sale_order_list dropdown-item"> <i class="fa-regular fa-eye"></i> View</a>
                                <?php if($edit == true): ?>
                                    <a href="<?php echo url('/').'/purchase/editSupplierForm/' . $row->s_id.'?m=1'; ?>" type="button" class="dropdown-item_sale_order_list dropdown-item BtnHide">
                                        <i class="fa-solid fa-pencil"></i> Edit
                                    </a>
                                <?php endif; ?>
                                <?php if($delete == true): ?>
                                    <a class="dropdown-item_sale_order_list dropdown-item BtnHide" type="button" onclick="delete_supp('<?php echo $row->s_id ?>')">
                                        <i class="fa-solid fa-trash"></i> Delete
                                    </a>
                                <?php endif; ?>

                                <a target="_blank" href="#" class="dropdown-item_sale_order_list dropdown-item">
                                    <i class="fa-solid fa-print"></i> Print
                                </a>
                                <?php if($row->acc_id == 0): ?>
                                    <?php if($createAccount == true): ?>
                                        <span id="ShowHide<?php echo $row->s_id ?>">
                                            <a type="button" class="dropdown-item_sale_order_list dropdown-item BtnHide" id="Btn<?php echo $row->id ?>" onclick="CreateAccount('<?php echo $row->acc_id ?>', '<?php echo $row->name ?>', '<?php echo $row->s_id ?>')">
...
//        $subitems = $subitems::get();
        ...
        foreach ($subitems as $row) {
            ...
            <tr id="RemoveTr<?php echo $row['id'] ?>" title="<?php echo $row['id'] ?>">
                ...
                <td class="hide">
                    <select name="item[]"><option <?php if ($row['finish_good']==1): ?>selected <?php endif ?> value="1,<?php echo $row['id'] ?>">Yes</option>
                        <option <?php if ($row['finish_good']==0): ?>selected <?php endif ?>  value="0,<?php echo $row['id'] ?>">No</option></select>
                </td>
            </tr>
            <?php
...
//        $subitems = $subitems::get();
    ...
    public function filterDemandVoucherList()
    {
        ...
        if ($selectVoucherStatus == '0' && empty($selectSubDepartmentId)) {
            $demandDetail = Demand::whereBetween('demand_date', [$fromDate, $toDate])->where('status', '!=', 0)->orderBy('id','desc')->get();
    ...
    {
        return view('Purchase.purchase_order', compact('id'));
    }
...
echo "aa"; die;
    ...
    public function filterGoodsReceiptNoteList(Request $request)
    {
        ...
        if ($grn_no != ''):
            ...
            if($selectVoucherStatus != 0) $query_string_second_part[] = "  AND grn_status ='$selectVoucherStatus'";
            ...
            $goodsReceiptNoteDetail = DB::select(DB::raw($query_string));
        endif;
    ...
    public function fetchDataOfMr(){
        $mr_id = $_GET['mr_id'];
        $material_requests = DB::connection('mysql2')->table('material_requests')->where([ ['id',$mr_id] , ['status', 1] ])->first();
        $material_request_data = DB::connection('mysql2')->table('material_request_datas')->where([ ['material_request_no',$material_requests->material_request_no] , ['status', 1] ])->get();
        $departments = new Department;
        $departments = $departments::where([['status', '=', '1'], ])->select('id','department_name')->orderBy('id')->get();
        return view('Purchase.AjaxPages.fetchDataOfMr', compact('material_requests','departments','material_request_data'));
    }
    ...
    function approve_grn()
    {
        ...
        try
        {
            ...
            $grn=DB::Connection('mysql2')->table('grn_data')
                ...
                ->select('grn_data.*', 'goods_receipt_note.supplier_id','goods_receipt_note.grn_date','goods_receipt_note.bill_date','goods_receipt_note.po_no','goods_receipt_note.supplier_invoice_no','goods_receipt_note.grn_no')->get();

            // Check if purchase return exists for this GRN
            $purchase_return = DB::Connection('mysql2')->table('purchase_return')
                ->where('grn_id', $id)
                ->where('status', 1)
                ->first();
            
            // Get purchase return data items if purchase return exists
            $return_items = [];
            if ($purchase_return) {
                $return_data = DB::Connection('mysql2')->table('purchase_return_data')
                    ->where('master_id', $purchase_return->id)
                    ->where('status', 1)
                    ->get();
                
                // Create array with grn_data_id as key and return_qty as value
                foreach ($return_data as $return_item) {
                    $return_items[$return_item->grn_data_id] = floatval($return_item->return_qty ?? 0);
                }
            }
            ...
            foreach($grn as $row):
                ...
                if ($accepted_qty > 0) {
                    ...
                    $stock['amount']=$proportional_net_amount;
            ...
            // Purchase voucher data - exclude returned items
            foreach($grn_data as $row):
                // Check if this item has a return quantity
                $return_qty = isset($return_items[$row->id]) ? $return_items[$row->id] : 0;
                $received_qty = floatval($row->purchase_recived_qty ?? 0);
                
                // Only add to purchase voucher if there's accepted quantity
                $accepted_qty = $received_qty - $return_qty;
                ...
                if ($accepted_qty > 0) {
                    ...
                    (
                        ...
                        'uom'=>0,
                        'qty'=>$accepted_qty, // Use accepted quantity
                        'rate'=>$row->rate,
...
// $content = json_decode($response->getBody(), true);
    ...
    public function get_stock_region_wise(Request $request)
    {
        ...
        $category = DB::Connection('mysql2')->select('select a.* ' . $category_column . ',b.uom,b.main_ic_id, b.pack_size, b.primary_pack_type,b.hs_code_id, b.color,b.item_code, b.sub_category_id from stock a
            inner JOIN
            subitem b
            ON
            b.id=a.sub_item_id
        ...
        return view('Store.AjaxPages.stock_report', compact('category', 'category_name', 'item_heading', 'location_heading', 'sub_item_heading', 'sub_item_des_heading', 'category_heading', 'quarintine', 'uom_data', 'packaging_data'));
    ...
    public function getSubCategoryList(Request $request)
    {
        ...
        $data = DB::Connection('mysql2')->table('sub_category as s')
            ->where('s.category_id', $category_id)
            ->where('s.status', 1)
            ->select('s.id', 's.sub_category_name')
            ->get();
        return response()->json($data);
    }

    public function getItemBySubCategory()
    {
    ...
    public function getSubItemByCategory(Request $request)
    {
        ...
        if ($item_id) {
            $product = Subitem::where(['status'=>1,'id'=>$item_id])->first();
            // dd($item_id , $products);
            $html = '<option selected value="'.$product->id.'">'.$product->sub_ic.'</option>';
        }
        else{
            ...
            {
                $html.= '<option value="'.$product->id.'">'.$product->sub_ic.'</option>';
            }
        }
    ...
    public function getSubItemByMainCategory(Request $request)
    {

        $html = '<option value="">Select</option>';

        $products = Subitem::where(['status'=>1,'main_ic_id'=>$request->category])->get();
        foreach($products as $product)
        {
            $html.= '<option value="'.$product->id.'">'.$product->sub_ic.'</option>';
        }
    ...
    public function get_po_status_data(Request $request)
    {
        ...
        //   LEFT JOIN grn_data b ON a.id=b.po_data_id
        //   where a.status=1
        //   and b.status=1
        ...
        //   GROUP by a.id HAVING a.purchase_approve_qty > sum(b.purchase_recived_qty)');
          ...
          $Data = DB::connection('mysql2')->select('
                SELECT 
                    ...
                    a.purchase_request_date,
                    b.warehouse_id,
                    a.purchase_approve_qty,
                    a.sub_item_id,
                    IFNULL(SUM(b.purchase_recived_qty), 0) AS purchase_recived_qty
                FROM 
                    purchase_request_data a
                INNER JOIN
                    purchase_request pr
                    ON pr.id = a.master_id
                LEFT JOIN 
                    grn_data b 
                    ON a.id = b.po_data_id AND b.status = 1
                WHERE 
                    a.status = 1
                    AND pr.purchase_request_status != 4
                    AND a.purchase_request_date BETWEEN "'.$FromDate.'" AND "'.$ToDate.'"
                GROUP BY 
                    a.id
                HAVING 
                    a.purchase_approve_qty > IFNULL(SUM(b.purchase_recived_qty), 0)
            ');
        }
        elseif($PoStatus == 2)
        {
    ...
    public function get_stock_location_wise2(Request $request)
    {
    ...
    function qc_submit(Request $request)
    {
        ...
        try
        {
            ...
            $data1 = array
            (
               'qc_qty'=>$request->input('reject_qty')[$key] ?? null,
    ...
    private function collectManualGatePassRows(Request $request, $companyId = null)
    {
    ...
    private function resolveGatePassUomName($uomId, $companyId = null)
    {
        $uomId = (int) $uomId;
        if ($uomId <= 0) {
            return '';
        ...
        $query = UOM::where('id', $uomId)->where('status', 1);
        if (!empty($companyId)) {
            $query->where('company_id', $companyId);
        }

        return (string) ($query->value('uom_name') ?? '');
    }

    private function resolveGatePassUomIdByName($uomName, $companyId = null)
    {
        $uomName = trim((string) $uomName);
        if ($uomName === '') {
            return null;
        }

        $query = UOM::where('uom_name', $uomName)->where('status', 1);
        if (!empty($companyId)) {
            $query->where('company_id', $companyId);
        }

        $uomId = $query->value('id');
        return !empty($uomId) ? (int) $uomId : null;
    }
    ...
    private function attachItemPurposes($itemRows, Request $request, $sourceType = null)
    {
        ...
        $purposes = $request->input('item_purpose', []);
        $hasPurposeInputs = collect($purposes)->contains(function ($value) {
        ...
        return collect($itemRows)->values()->map(function ($row, $index) use ($purposes) {
            if (empty($row->source_id) && empty($row->source_item_id)) {
                return $row;
            }

            $rowKey = $row->source_row_key ?? $row->source_item_id ?? $index;
            $row->purpose = trim((string) ($purposes[$rowKey] ?? ''));
            return $row;
        });
    }
    ...
    {
        if ((string) $sourceType === '3') {
            return $itemRow->party_id ?? $fallbackPartyId;
        }

        return $itemRow->customer_id ?? $itemRow->party_id ?? null;
    }

    private function loadGatePassSourceItems(array $sourceIds, $type)
    {
        if (empty($sourceIds)) {
            return collect();
        }
        ...
        if ((string) $type === '1') {
            return DB::connection('mysql2')->table('sales_tax_invoice_data as a')
                ->leftJoin('subitem as si', 'si.id', '=', 'a.item_id')
                ->leftJoin('sales_tax_invoice as sti', 'sti.id', '=', 'a.master_id')
                ...
                ->select(
                    ...
                    'a.id as source_row_key',
                    'sti.buyers_id as customer_id',
        ...
        return DB::connection('mysql2')->table('delivery_note_data as a')
            ->leftJoin('subitem as si', 'si.id', '=', 'a.item_id')
            ->leftJoin('delivery_note as dn', 'dn.id', '=', 'a.master_id')
    ...
    private function applyGatePassDetailsToSourceItems(array $sourceItems, $gatePassDetails)
    {
        if (empty($sourceItems) || empty($gatePassDetails)) {
            return $sourceItems;
        }

        $detailsBySource = [];
        foreach ($gatePassDetails as $detail) {
            $detailsBySource[(string) ($detail->source_id ?? '')][] = $detail;
        }

        foreach ($sourceItems as $sourceId => &$rows) {
            $sourceKey = (string) $sourceId;
            if (empty($detailsBySource[$sourceKey])) {
                continue;
            }
            ...
            foreach ($rows as &$row) {
                $matchedIndex = null;
                $rowItemId = (string) ($row['item_id'] ?? '');
                $rowKey = (string) ($row['source_row_key'] ?? '');

                foreach ($detailsBySource[$sourceKey] as $index => $detail) {
                    if (in_array($index, $usedIndexes, true)) {
                        continue;
                    }

                    if ($rowKey !== '' && (string) ($detail->source_row_key ?? '') === $rowKey) {
                        $matchedIndex = $index;
                        break;
                    }

                    if ((string) ($detail->source_item_id ?? '') === $rowItemId) {
                        $matchedIndex = $index;
                        break;
                    }
                }
                ...
                $detail = $detailsBySource[$sourceKey][$matchedIndex];
    ...
    public function getGatePassSourceItems(Request $request)
    {
        $type = (string) $request->input('type', '');
        $sourceIds = $this->normalizeSelectedIds($request->input('source_ids', []));

        if (!in_array($type, ['1', '2'], true) || empty($sourceIds)) {
        ...
        $rows = $this->loadGatePassSourceItems($sourceIds, $type);
        $grouped = [];

        foreach ($rows as $row) {
            $grouped[$row->master_id][] = [
    ...
    private function markGatePassSources(array $sourceIds, $type, $status)
    {
        if (empty($sourceIds)) {
            return;
        }
        ...
        if ((string) $type === '1') {
            DB::connection('mysql2')->table('sales_tax_invoice')
                ...
                ->update(['gate_pass_status' => $status]);
    ...
    private function gatePassHasColumn($column)
    {
        return Schema::connection('mysql2')->hasColumn('gate_pass', $column);
    }

    private function gatePassDataHasColumn($column)
    {
    ...
    public function createGatePassForm(Request $request){
        ...
        $selectedSalesInvoiceIds = $this->normalizeSelectedIds(old('sales_invoice_id', []));
        $selectedDeliveryNoteIds = $this->normalizeSelectedIds(old('delivery_note_id', []));
        $description = old('description', '');
        $selectedPartyId = old('party_id', '');
        $gatePassDate = old('gate_pass_date', date('Y-m-d'));
        $gatePassTime = old('gate_pass_time', date('H:i'));
        $vehicleNo = old('vehicle_no', '');
        $vehicleType = old('vehicle_type', '');
        $driverName = old('driver_name', '');
        $transporterName = old('transporter_name', '');
        $vehicleContact = old('vehicle_contact', '');
        ...
        if ($this->gatePassDataHasColumn('uom')) {
            $manualPartySelectColumns[] = 'uom';
        ...
        if (!empty($directSaleInvoiceIds)) {
            $invoiceItems = $this->loadGatePassSourceItems($directSaleInvoiceIds, 1);

            foreach ($invoiceItems as $item) {
                $directSaleInvoiceItems[$item->master_id][] = [
                    'source_id' => $item->source_id,
                    'source_row_key' => $item->source_row_key ?? null,
        ...
        if (!empty($deliveryNoteIds)) {
            $dnItems = $this->loadGatePassSourceItems($deliveryNoteIds, 2);
    ...
    public function editGatePassForm($id, Request $request)
    {
        ...
        $gatePassDate = $gatePassEdit->gate_pass_date ?? date('Y-m-d');
        $gatePassTime = $gatePassEdit->gate_pass_time ?? date('H:i');
        $vehicleNo = $gatePassEdit->vehicle_no ?? '';
        $vehicleType = $gatePassEdit->vehicle_type ?? '';
        $driverName = $gatePassEdit->driver_name ?? '';
        ...
        $manualGatePassItems = DB::connection('mysql2')->table('gate_pass_data')
                ...
                ->where(function ($query) {
                    $query->where('is_editable', 1)
                        ->orWhereNull('source_item_id')
                        ->orWhere('source_item_id', 0);
                })
                ->orderBy('id', 'ASC')
                ->get($manualPartySelectColumns);

        $manualGatePassItems = $manualGatePassItems->map(function ($manualItem) use ($m) {
            $savedUomName = trim((string) ($manualItem->uom ?? ''));
            if ($savedUomName !== '') {
                $manualItem->uom_id = $this->resolveGatePassUomIdByName($savedUomName, $m) ?? '';
                $manualItem->uom_name = $savedUomName;
            } elseif (!empty($manualItem->uom_id)) {
                $manualItem->uom_name = $this->resolveGatePassUomName($manualItem->uom_id, $m);
            }
        ...
        $directSaleInvoices = DB::connection('mysql2')
            ...
            ->select('sti.id', 'sti.gi_no', 'sti.gi_date', 'sti.buyers_id', 'sti.gate_pass_status', 'c.name as customer_name')
            ->orderBy('sti.id', 'DESC')
            ->get();
    ...
    public function viewGatePassListAjax(Request $request)
    {
        ...
        return response()->json([
            ...
            'table_html' => view('GatePass.ajax_gate_pass_table', compact('gatePasses', 'm'))->render(),
            'page_info' => [
    ...
    public function viewGatePassDetailAjax(Request $request)
    {
        $id = $request->input('id');
        $m = $request->input('m', $_GET['m'] ?? Session::get('run_company'));
        CommonHelper::companyDatabaseConnection($m);
        ...
        if ((int) $gatePass->gate_pass_type === 1) {
            $sourceTypeLabel = 'Direct Sale Invoice';
        } elseif ((int) $gatePass->gate_pass_type === 2) {
            $sourceTypeLabel = 'Delivery Note';
        }

        $partyName = '';
        if ((int) $gatePass->gate_pass_type === 3 && !empty($gatePass->party_id)) {
            $partyName = DB::connection('mysql2')->table('customers')
        ...
        return view('GatePass.ajax_gate_pass_detail', compact('gatePass', 'items', 'sourceTypeLabel', 'partyName', 'm'));
    }
    ...
    public function storeGatePass(Request $request)
    {
        ...
        $salesInvoiceIds = $this->normalizeSelectedIds($request->input('sales_invoice_id', []));
        $deliveryNoteIds = $this->normalizeSelectedIds($request->input('delivery_note_id', []));
        $partyId = $request->input('party_id');

        if ((string) $request->gate_pass_type === '1' && empty($salesInvoiceIds)) {
            CommonHelper::reconnectMasterDatabase();
            return redirect()->back()->withInput()->with('error', 'Please select at least one direct sale invoice.');
        }

        if ((string) $request->gate_pass_type === '2' && empty($deliveryNoteIds)) {
            CommonHelper::reconnectMasterDatabase();
            return redirect()->back()->withInput()->with('error', 'Please select at least one delivery note.');
        }
        ...
        try {
            ...
            if ((string) $request->gate_pass_type === '1') {
                ...
                $alreadyUsed = $sources->first(function ($source) {
                    return !empty($source->gate_pass_status);
                });
                if (!empty($alreadyUsed)) {
                    DB::connection('mysql2')->rollBack();
                    CommonHelper::reconnectMasterDatabase();
                    return redirect()->back()->with('error', 'Gate pass already created for one of the selected direct sale invoices.');
                }

                $sourceNo = $sources->pluck('gi_no')->filter()->implode(', ');
                $sourceId = $sourceIds[0] ?? null;
                $itemRows = $this->loadGatePassSourceItems($sourceIds, 1);
          } elseif ((string) $request->gate_pass_type === '2') {
              $sourceIds = $deliveryNoteIds;
              $sourceIdsText = implode(',', $sourceIds);
                ...
                if (!empty($alreadyUsed)) {
                    DB::connection('mysql2')->rollBack();
                    CommonHelper::reconnectMasterDatabase();
                    return redirect()->back()->with('error', 'Gate pass already created for one of the selected delivery notes.');
                }
                  $sourceNo = $sources->pluck('gd_no')->filter()->implode(', ');
                  $sourceId = $sourceIds[0] ?? null;
                  $itemRows = $this->loadGatePassSourceItems($sourceIds, 2);
              } else {
              ...
              if (!empty($manualRows['rows'])) {
                  $itemRows = $itemRows->concat(collect($manualRows['rows']))->values();
              }
            ...
            if ($this->gatePassHasColumn('source_ids')) {
                $gatePassData['source_ids'] = $sourceIdsText;
            }
            if ($this->gatePassHasColumn('party_id')) {
                $gatePassData['party_id'] = $this->resolveGatePassPartyId($request->gate_pass_type, $itemRows->first(), $partyId);
            }

            $gatePassId = DB::connection('mysql2')->table('gate_pass')->insertGetId($gatePassData);

            foreach ($itemRows as $itemRow) {
                $gatePassDetail = [
                    'gate_pass_id' => $gatePassId,
                    'source_type' => $request->gate_pass_type,
                    'source_id' => $itemRow->source_id ?? null,
                    'source_no' => !empty($itemRow->source_id) ? $sourceNo : null,
                ...
                if ($uomName === '') {
                    $uomName = $this->resolveGatePassUomName($itemRow->uom_id ?? 0, $m);
                }
            ...
            if ((string) $request->gate_pass_type === '1') {
                $this->markGatePassSources($sourceIds, 1, 1);
            } elseif ((string) $request->gate_pass_type === '2') {
                $this->markGatePassSources($sourceIds, 2, 1);
            }
    ...
    public function updateGatePass(Request $request, $id)
    {
        $validated = $request->validate([
            'gate_pass_type' => 'required|in:1,2,3',
            'description' => 'nullable|string',
        ...
        try {
            $gatePass = DB::connection('mysql2')->table('gate_pass')->where('id', $id)->where('company_id', $m)->first();
            if (empty($gatePass)) {
                DB::connection('mysql2')->rollBack();
                CommonHelper::reconnectMasterDatabase();
                return redirect()->back()->with('error', 'Gate pass not found.');
            ...
            if ((string) $request->gate_pass_type === '1') {
                $sourceIds = $this->normalizeSelectedIds($request->input('sales_invoice_id', []));
                $sourceIdsText = implode(',', $sourceIds);
                $sources = DB::connection('mysql2')->table('sales_tax_invoice')
                    ->whereIn('id', $sourceIds)
                    ->whereIn('is_official', CommonHelper::getOfficialScopeArray())
            ...
            DB::connection('mysql2')->table('gate_pass')->where('id', $id)->update($gatePassData);
    ...
    public function deleteGatePass($id, Request $request)
    {
        ...
        try {
            ...
            DB::connection('mysql2')->table('gate_pass_data')->where('gate_pass_id', $id)->update(['status' => 2]);
            DB::connection('mysql2')->table('gate_pass')->where('id', $id)->update(['status' => 2]);
            ...
            DB::connection('mysql2')->rollBack();
            CommonHelper::reconnectMasterDatabase();
            return redirect()->back()->with('error', 'Gate pass could not be deleted.');
        }
    }
